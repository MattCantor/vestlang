// Static empty-window analysis for a gate's BEFORE/AFTER date constraints.
//
// A gate combines BEFORE/AFTER comparisons against fixed dates. Each one cuts the
// number line on one side; an AND intersects the cuts, an OR unions them. When the
// result admits no date at all, the gate can never be satisfied — no firing of any
// referenced event, on any date, lands in an empty window. This module computes
// that satisfiable set, so a caller can ask `isEmptySatisfiableSet(condition)`.
//
// It reads only the gate's *statically datable* anchors (plain fixed DATEs).
// Anything it can't date — an event base, an offset date, a gated anchor —
// contributes the full line `[{}]`, so the analysis can only ever *under*-report:
// it never calls a still-satisfiable gate empty. Two consumers share it: the linter
// (which reports the empty window as an error) and the evaluator (which classifies
// the gated node impossible).
...
// datable without committing to a policy this analysis doesn't own) and no
// condition of its own (a gated anchor isn't a fixed point either).
...
// Used for the running merge in the sweep, where every conjunct narrows a single
// window.
export const intersect = (a: Window, b: Window): Window => {
  const lower =
    a.lower && b.lower
...
export const nonEmpty = (w: Window): boolean => !isEmpty(w);
...
// Map a condition to its satisfiable set. The per-edge strictness rule (bare
...
// the analysis sound: it can only under-report, never false-positive on something
// still satisfiable.
export const satisfiableSet = (c: Condition): IntervalSet => {
  switch (c.type) {
    case "ATOM": {
      const { constraint } = c;
...
// The headline question: does this gate's date window admit no date at all? An
// empty satisfiable set means every alternative pins the date to nothing, so the
// gate is a static contradiction. Both consumers route off this.
export const isEmptySatisfiableSet = (c: Condition): boolean =>
  satisfiableSet(c).length === 0;
