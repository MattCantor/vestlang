...
export function evaluateProgramAsOf(
  ...
  // No context build here: evaluateProgram (below) funnels ctx_input through
  // createEvaluationContext itself, so the grant-quantity / date / sentinel guards
  // all still run — and the installment cap with them. We only need two raw fields
  // off the input (grantQuantity for the allocator, asOf for the partition), and
  // both ride through the builder unchanged, so reading them straight off ctx_input
  // is the same value without the duplicate validation pass.
  ...
  const draw = claimAllocator(ctx_input.grantQuantity);
  const programQuantity = program.reduce(
    (n, s) => n + draw(amountToFraction(s.amount, ctx_input.grantQuantity)),
    0,
  );
