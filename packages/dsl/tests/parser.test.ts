...
// Helpers
const makeDuration = (value: number, unit: "DAYS" | "MONTHS"): Duration => ({
  type: "Duration",
  value,
  unit,
});
const makeDate = (value: string): DateAnchor => ({ type: "Date", value });
const makeEvent = (value: string): EventAnchor => ({ type: "Event", value });
...
const constrainedBefore = (props: constrainedProps): ConstrainedAnchor => ({
  type: "Constrained",
  base: props.base,
  constraints: [
    { type: "Before", i: props.target, strict: props.strict },
  ] satisfies BaseConstraint[],
});

const constrainedAfter = (props: constrainedProps): ConstrainedAnchor => ({
  type: "Constrained",
  base: props.base,
  constraints: [
    { type: "After", i: props.target, strict: props.strict },
  ] satisfies BaseConstraint[],
});

const constrainedBetween = (
  base: BareAnchor,
  startTarget: BareAnchor,
  startStrict: boolean,
  endTarget: BareAnchor,
  endStrict: boolean,
): ConstrainedAnchor => ({
  type: "Constrained",
  base,
  constraints: [
    { type: "After", i: startTarget, strict: startStrict },
    { type: "Before", i: endTarget, strict: endStrict },
  ] satisfies BaseConstraint[],
});
...
describe("vestlang PEG grammar", () => {
  ...
  describe("Schedule basics", () => {
    ...
    it("explicit 0-day durations stay as Duration, not Zero", () => {
      ...
      const s = ast.expr as ASTSchedule;
      expect(s.over).toEqual(makeDuration(0, "DAYS"));
      expect(s.every).toEqual(makeDuration(0, "DAYS"));
    });
    ...
    it("normalizes weeks→days and years→months", () => {
      ...
      const s = ast.expr as ASTSchedule;
      expect(s.over).toEqual(makeDuration(24, "MONTHS")); // 2y → 24m
      expect(s.every).toEqual(makeDuration(7, "DAYS")); // 1w → 7d
    });
  });
  ...
  describe("FROM term (Date/Event, Earlier/Later, predicates)", () => {
    ...
    it("supports QualifiedAtom with BEFORE / AFTER / BETWEEN", () => {
      ...
      const beforeDate = makeDate("2026-01-01");
      const before = parse(
        "VEST SCHEDULE FROM DATE 2025-01-01 BEFORE EVENT milestone",
      ) as ASTStatement;
      ...
      const strictlyBefore = parse(
        "VEST SCHEDULE FROM DATE 2025-01-01 STRICTLY BEFORE EVENT milestone",
      ) as ASTStatement;
      ...
      const after = parse(
        "VEST SCHEDULE FROM EVENT milestone AFTER DATE 2025-01-01",
      ) as ASTStatement;
      ...
      const between = parse(
        "VEST SCHEDULE FROM EVENT milestone AFTER DATE 2025-01-01 AND BEFORE DATE 2026-01-01",
      ) as ASTStatement;
      expect((between.expr as ASTSchedule).from).toEqual(
        constrainedBetween(event, afterDate, false, beforeDate, false),
      );
    });
  });
