// Analytic-CORE sweep over the widened (v2) grid, MINUS_ONE-original subset
// reported separately exactly like the baseline widened run. Clone of
// runSpikeWidened.ts pointing at the production stage-2b core.
//
//   node packages/inferrer/experiments/runCoreWidened.ts [outDir]
//
// The core is imported from the built dist, so build first:
//   pnpm --dir packages/inferrer build
//
// Writes <outDir>/core-analytic-widened.json: counts (main + MINUS_ONE),
...
writeFileSync(
  join(outDir, "core-analytic-widened.json"),
  JSON.stringify(report, null, 2) + "\n",
);
console.log(JSON.stringify(report.grid), JSON.stringify(analyticStats));
