// Locks in resolution of the vendored OCF types through their `file:` local
// package. The guarantee is type-level: the final package specifier must resolve
// under the whole-repo typecheck, and each name must resolve to a concrete type
// rather than silently collapse to `any` if resolution ever breaks. `pnpm test`
// runs this green regardless (the imports are type-only); `pnpm typecheck` is
// what enforces it.
...
// directive fails the build and the vendored surface needs a fresh look.
import type { OCFVestingStatement } from "@opencaptablecoalition/ocf-types";

it("resolves the OCF types package and its expected export surface", () => {
