...
| `absenceAssumptions` | array | Events the resolves-to reading is assuming stayed absent — each `{ eventId, through, direction, inclusive, message }`. Direction-aware: `direction` (`"before"`/`"after"`) and `inclusive` say which side of `through` a dangerous firing of `eventId` falls on, so the `message` reads "did not occur before/after `through`" (a `BEFORE`/EARLIER OF watch names the before side, an `AFTER`/LATER OF the after side). A later/backdated firing on that side could change the result. Empty for a date-only or fully-fired schedule. |
