...
cliff gated on an event stores as one **`template`** — a time `cliff` (here a zero-length
hold) plus the `ipo` gate carried in `event_condition` — and resolves to **`template`** too,
just held pending until `ipo` fires (`representable: true`, `pending: true`). Both verdicts
agree; the projection stays empty until the event lands (see [the two
verdicts](./evaluation.md#the-two-verdicts)):
