...
- Scenario modeling: call vestlang_evaluate or vestlang_evaluate_as_of with
  ...
  { eventId, through, direction, inclusive, message } — direction-aware: the
  dangerous firing falls on the before/after side of \`through\`), whose later or
  backdated firing could change the
...
export function createServer(): McpServer {
  ...
  server.registerTool(
    ...
    async (params) => {
      ...
      // reason + inferred DSL). `breakdownResidual`/`breakdownNote` ride along
      // with the breakdown — present only when there's a breakdown to reconcile.
      return okResult({
        ...result.view,
        breakdown: result.breakdown,
        ...(result.recovered ? { recovered: result.recovered } : {}),
        ...(result.breakdownResidual !== undefined
          ? {
              breakdownResidual: result.breakdownResidual,
