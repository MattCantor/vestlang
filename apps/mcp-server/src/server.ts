...
- Natural language → vestlang: vestlang://docs/grammar is the authoritative
  ...
  diagnostic, fix and re-validate. When the description comes with a few known
  figures (a disclosed tranche, fiscal-year-end balances), fetch
  vestlang://docs/authoring and verify the draft against them.
- Narrative + a few known figures → verified vestlang: when the description is a
  loose narrative (a footnote tranche, year-end balances) rather than a full
  stream, fetch vestlang://docs/authoring for the propose→verify→refine recipe —
  it maps narrative phrases to observation kinds and walks the verify loop.
...
- Scenario modeling: call vestlang_evaluate or vestlang_evaluate_as_of with
  ...
  the whole schedule with TWO verdicts: \`storable\` (what a record keeper could
  hold, computed without reading firings) and \`resolvesTo\` (what it works out to
  given the events you passed). They can differ — a gated start is a storable template that may
...
- Tranche array → vestlang: call vestlang_infer_schedule on an array of
  ...
  in a fixed preference order wins. A stream no single schedule can read may still
  come back as a compact PLUS cover of concurrent layers, and anything still
  unrecognized falls back to a literal per-date list; diagnostics.recoveryMode says
  which happened. The result also carries a \`context\` object keyed as
  vestlang_evaluate's parameters (grant_date, grant_quantity, vesting_day_of_month,
  events); pass these directly to vestlang_evaluate alongside the returned DSL to
  re-evaluate it faithfully (the day-of-month is not encoded in the DSL). Note
  vestlang_infer_schedule assumes the tranches are the complete grant; for sparse
  partial evidence (a few footnote tranches, a year-end balance) use the authoring
  recipe (vestlang://docs/authoring) with vestlang_verify_observations instead.
    ...
    async ({ tranches, grant_date }) => {
      try {
        ...
        return okResult({
          ...result,
          context: {
            grant_date: result.context.grantDate,
            grant_quantity: result.context.grantQuantity,
