...
The split decouples *runtime-aware resolution + intent* (extended) from *exact allocation* (core), eliminating the duplicated engine and giving OCF-Tools a real reference implementation. Crucially, it reframes vestlang: **not** a structurally-richer schedule format (that can't survive the round-trip), but the **contingency front-end + inferrer + conformance classifier** that maps the interchange's own expressibility boundary — and falls back gracefully when a schedule lands outside it.
